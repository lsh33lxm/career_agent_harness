...
- `BLOCKED_EXTERNAL_ACTION`：第三方插件的许可证、条款、immutable commit、依赖和网络域名未核验前只能进入 quarantined/preview；真实 marketplace 下载与第三方 worker 执行不启用。
...
- `QUARANTINED_EXTERNAL_SOURCE`：JobSpy、WebMagic/JobHunter 和 AI Job Search portal 尚未完成许可证、robots/ToS、登录边界、网络权限与 credentials 审核；Slice D 只启用 manual/offline source。
