...
def test_empty_successful_snapshot_does_not_advance_lifecycle_missing_count(
    tmp_path: Path,
) -> None:
    class EmptyAfterFirstSource(ManualJobSource):
        source_id = "job-source-empty-after-first"

        def __init__(self) -> None:
            super().__init__(
                (
                    RawJobRecord(
                        source_ref="https://example.com/jobs/kept",
                        raw_text='{"title":"Kept","company":"Example"}',
                    ),
                )
            )
            self.empty = False
        ...
        def search(self, query: str) -> tuple[RawJobRecord, ...]:
            return () if self.empty else self.records

    service = _service(tmp_path)
    source = EmptyAfterFirstSource()
    service.collect(source, query="intern")
    source.empty = True

    assert service.collect(source, query="intern") == ()
    observation = service.lifecycle_observations()[0]
    assert observation.consecutive_missing == 0
    assert observation.status.value == "active"


def test_failed_snapshot_does_not_advance_existing_lifecycle(
    tmp_path: Path,
) -> None:
    class FailingAfterFirstSource(ManualJobSource):
        source_id = "job-source-failed-after-first"
    ...
    service = _service(tmp_path)
    source = FailingAfterFirstSource()
    service.collect(source, query="intern")
    source.fail = True
...
def test_duplicate_successful_snapshot_is_idempotent_for_lifecycle(
    tmp_path: Path,
) -> None:
    service = _service(tmp_path)
    source = ManualJobSource(
        (
            RawJobRecord(
                source_ref="https://example.com/jobs/stable",
                raw_text='{"title":"Stable","company":"Example"}',
            ),
        )
    )

    service.collect(source, query="stable")
    service.collect(source, query="stable")
    observation = service.lifecycle_observations()[0]
    assert observation.consecutive_missing == 0
    assert observation.status.value == "active"
