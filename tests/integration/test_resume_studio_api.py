...
@pytest.mark.asyncio
async def test_magic_resume_shape_survives_base_save_round_trip(tmp_path: Path) -> None:
    transport = httpx.ASGITransport(app=_app(tmp_path))
    source = {
        "title": "Magic-compatible resume",
        "basics": {"name": "Minnn", "headline": "Platform Engineer"},
        "sections": [
            {"id": "work", "hidden": False},
            {"id": "projects", "hidden": True},
            {"id": "education", "hidden": False},
        ],
        "education": [{"id": "edu_1", "school": "Example University", "degree": "MSc"}],
        "work": [{"id": "work_1", "company": "Example", "details": ["Built platform"]}],
        "projects": [{"id": "project_1", "name": "Career Harness", "link": "https://example.com"}],
        "template": {"id": "modern", "locale": "zh-CN"},
        "futureField": {"schemaVersion": 2, "keep": True},
    }
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        saved = await client.post(
            "/api/v1/resume/bases",
            headers={**AUTH, "X-Idempotency-Key": "magic-round-trip-001"},
            json={
                "command_id": "magic_round_trip_command",
                "resume_id": "resume_magic",
                "candidate_id": "candidate_001",
                "sections": source,
            },
        )
        history = await client.get("/api/v1/resumes/resume_magic/bases", headers=AUTH)

    assert saved.status_code == 201
    persisted = saved.json()["sections"]
    assert list(persisted) == list(source)
    assert persisted == source
    assert history.status_code == 200
    assert history.json()[0]["sections"] == source
...
async def test_resume_base_history_restore_is_append_only(tmp_path: Path) -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        undone = await client.post(
            "/api/v1/resume/undo",
            headers={**AUTH, "X-Idempotency-Key": "undo-base-history-001"},
            json={
                "command_id": "undo_base_history_001",
                "resume_id": "resume_001",
