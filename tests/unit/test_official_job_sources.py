...
def test_official_fetch_retries_transient_failure_and_applies_rate_limit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = OfficialCampusJobSource(TENCENT_CAMPUS, fixture_html="<html></html>")
    source.config = replace(TENCENT_CAMPUS, rate_limit_ms=1, max_retries=1)
    calls = 0
    ...
    monkeypatch.setattr("career_harness.adapters.official_job_sources.urlopen", fake_open)
    assert source._fetch(TENCENT_CAMPUS.landing_url)[0] == "<html></html>"
    assert calls == 2
