...
| M3h | Add Application revisions and immutable Outcome records | Completed in `4b0bb68`; additive 0012, no ATS action, form payload or legacy backfill | Tested `0011 -> 0012 -> 0011 -> 0012` on disposable DB. |
...
- JobRequirement promotion must validate exact source Evidence and accepted official Capability
  membership; typed/generic state, event and idempotency roll back atomically on failure; legacy
  Opportunity rows remain readable until approved reconciliation.
...
- Match assessments, requirement results and gaps are immutable; gaps pin exact requirement
  revisions; assessment typed rows, generic revision, event and idempotency roll back atomically;
  replay uses only the stored manifest and exact reads.
- Resume patches pin exact base/fact/evidence/requirement refs and require USER review; resume
  revisions are immutable and content-hashed; dangling refs fail loud at the write path.
