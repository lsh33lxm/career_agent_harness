...
from career_harness.core.resume import (
    ...
    RevisionRef,
    preserve_validated_resume_json,
)
...
class ResumeBaseRestoreRequest(FrozenModel):
    command_id: OpaqueId
    resume_id: OpaqueId
...
def create_resume_studio_router(api: ResumeStudioApi) -> APIRouter:
    ...
    def save_base(
        ...
        try:
            sections = preserve_validated_resume_json(request.sections)
    ...
    @router.post("/base-restore", response_model=ResumeBase, status_code=status.HTTP_201_CREATED)
    def restore_base(
        request: ResumeBaseRestoreRequest,
        idempotency_key: IdempotencyHeader,
    ) -> ResumeBase:
        command = _command(
            command_id=request.command_id,
