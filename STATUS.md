...
- 现场只读验证：列表返回 77 条校园/实习岗位；首条详情成功读取，标题为“海外市场运营实习生（日本市场）”，地点“广东深圳市”，发布日期 `2026-08-12`，解析出 22 条任职/加分要求，详情 URL 与岗位 ID 精确匹配。
- `tests/unit/test_official_job_sources.py` 官方来源专项 12 项通过；适配器与测试 Ruff 通过。原文只在现场响应内处理，不写入仓库或发布快照。

## 2026-09-25 Resume Studio 结构化字段编辑

- Resume Studio 在自有 JSON 往返编辑器上增加标量字段控件；字符串、数字、布尔和空值可直接修改，复杂对象/数组与未知顶层字段继续由 JSON 编辑器保留。
- 字段编辑只更新本地待审核草稿，提交仍必须引用证据并经过现有 Resume Patch Review 与不可变 ResumeRevision 流程，不覆盖基础简历。
- 前端全量回归 30 个测试文件、86 项通过；Resume Studio 后端专项 13 项通过；桌面生产构建通过（Vite 仍提示主 bundle 超过 500 kB）。

## 2026-09-25 申请看板拖拽与面试月视图

- 申请看板卡片支持拖拽；准备状态只调用既有 preparation-state 命令，投递后的状态只调用既有 `/applications/{id}/state` 合法转移 API。失败不乐观改列，保留原卡片并显示中文错误。
- 面试区域增加本机时区月视图、列表视图切换和前后月份导航；事件仍从 Interview 读模型生成，点击月视图事件返回可操作列表，`.ics` 导出继续使用稳定 UID 和 UTC 时间。
- `ApplicationsPage.test.tsx` 专项 3 项通过，包含拖拽状态转移、月视图和 `.ics` 导出；桌面生产构建通过。
- 后端申请命令与面试服务回归 14 项通过；前端全量回归 30 个测试文件、87 项通过。
