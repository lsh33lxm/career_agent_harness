...
class OfficialCampusJobSource:
    ...
    def _throttle(self) -> None:
        wait_seconds = self.config.rate_limit_ms / 1000 - (time.monotonic() - self._last_request_at)
        if wait_seconds > 0:
    ...
    def _open(self, request: Request):
        ...
        for attempt in range(self.config.max_retries + 1):
            self._throttle()
            try:
                return urlopen(request, timeout=self.config.timeout_seconds)  # noqa: S310
            except (HTTPError, URLError, TimeoutError) as error:
                last_error = error
                if attempt >= self.config.max_retries:
                    raise
                time.sleep(min(0.25 * (2**attempt), 1.0))
        raise RuntimeError("official source request failed") from last_error
