...
import { admitStagedJob, getOfficialJobDetail, listJobListingLifecycle, listJobRequirements, listJobSourcePolicies, proposeJobRequirement, reviewJobRequirement, searchOfficialJobs } from "../api/jobRadar";
...
describe("OfficialSourcesPanel", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(getOfficialJobDetail).mockResolvedValue({
      source_id: "official-cn-tencent-campus", source_ref: "", captured_at: "2026-09-25T00:00:00Z",
      raw_text: JSON.stringify({ title: "平台工程实习生", company: "腾讯", description: "职位描述", requirements: ["熟悉 Python"] }),
      normalized: { title: "平台工程实习生", company: "腾讯", location: "深圳", remote: null, salary: null, requirements: ["熟悉 Python"], source_url: "" },
  ...
  it("admits a staged job and renders reviewable JD candidates", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "读取官方岗位" }));
    fireEvent.click(await screen.findByRole("button", { name: "加入并审核 JD" }));
    await waitFor(() => expect(getOfficialJobDetail).toHaveBeenCalledWith({ source_id: "official-cn-tencent-campus", source_ref: "https://join.qq.com/job/2" }));
    expect(await screen.findByDisplayValue("熟悉 Python")).toBeTruthy();
    expect(screen.getByText(/proposed/)).toBeTruthy();
    expect(screen.getByLabelText("平台工程实习生 完整官方 JD").textContent).toContain("职位描述");
