...
it("creates a user target profile, renders preview, shows ATS gaps and authenticates download", async () => {
  ...
  const fetcher = vi.fn()
    ...
    .mockResolvedValueOnce(response([
      { opportunity: { entity_id: "opp_001", revision: 3, schema_version: 1, state: "preparing" }, job: { job_id: "job_001", revision: 2 }, suggested_priority: null, user_priority: null },
    ]))
    .mockResolvedValueOnce(response([
      { requirement_id: "req_accepted", revision: 2, job: { job_id: "job_001", revision: 2 }, requirement_text: "熟悉 Python", importance: "required", required_scopes: ["understand"], source_evidence_refs: ["evidence_1"], status: "accepted", capability_id: "cap_1", graph_version_id: "graph_1", review_reason: "已核对", reviewed_at: "now" },
      { requirement_id: "req_pending", revision: 1, job: { job_id: "job_001", revision: 2 }, requirement_text: "了解云平台", importance: "preferred", required_scopes: ["understand"], source_evidence_refs: ["evidence_2"], status: "proposed", capability_id: null, graph_version_id: null, review_reason: null, reviewed_at: null },
    ]))
    .mockResolvedValueOnce(response({
      target_profile_id: "target_profile_001", resume_id: "resume_001",
      title: "Platform Engineer", company: null, opportunity_id: "opp_001",
      opportunity_revision: 3, requirement_refs: [{ entity_id: "req_accepted", revision: 2 }], keyword_gaps: [],
  ...
  fireEvent.focus(screen.getByLabelText("绑定机会版本"));
  expect(await screen.findByRole("option", { name: /opp_001/ })).toBeTruthy();
  fireEvent.change(screen.getByLabelText("绑定机会版本"), { target: { value: "opp_001" } });
  expect(await screen.findByText(/已审核要求：1 条/)).toBeTruthy();
  ...
  const profileRequest = JSON.parse(fetcher.mock.calls[5][1].body as string) as Record<string, unknown>;
  expect(profileRequest).toMatchObject({ opportunity_id: "opp_001", opportunity_revision: 3, requirement_refs: [{ entity_id: "req_accepted", revision: 2 }] });
