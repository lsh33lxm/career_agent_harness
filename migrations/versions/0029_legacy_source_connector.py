"""Allow governed Legacy Agent Radar source connectors."""

from collections.abc import Sequence

from alembic import op

revision: str = "0029_legacy_source_connector"
down_revision: str | None = "0028_source_connectors"
...
def downgrade() -> None:
    op.execute(
        ...
        "(SELECT connector_id FROM source_connector WHERE connector_type='legacy_agent_radar')"
    )
    op.execute(
        "DELETE FROM source_resource WHERE connector_id IN "
        "(SELECT connector_id FROM source_connector WHERE connector_type='legacy_agent_radar')"
    )
    op.execute("DELETE FROM source_connector WHERE connector_type='legacy_agent_radar'")
    _replace_type_constraint("'local_folder'")
