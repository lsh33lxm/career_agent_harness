...
- 结果洞察：历史页可调用 `/api/v1/outcome-insights/applications/{application_id}/offer-preparation` 与 `/api/v1/outcome-insights/rejection-pattern`；两者只读取已记录 Outcome，生成 Knowledge proposal，不自动修改申请、简历或 Career Core。
