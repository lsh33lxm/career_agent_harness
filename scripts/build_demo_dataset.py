...
def _location(value: str) -> str:
    ...
    try:
        ...
        if isinstance(parsed, list):
            return "、".join(str(item).strip() for item in parsed if str(item).strip())[:120] or "地点待确认"
    except json.JSONDecodeError:
        pass
    return value.strip()[:120]


def build(source: Path, output: Path, *, limit: int = 24, seed: str = "ach-demo-v1") -> dict[str, Any]:
    ...
    (output / "jobs.json").write_text(json.dumps(selected, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    manifest = {
        ...
        "seed": seed,
        "excluded_fields": ["original_url", "responsibilities_raw", "requirements_raw", "preferred_raw", "original_archive_path", "source_table", "source_id"],
...
def main() -> None:
    ...
    print(json.dumps(build(args.source, args.output, limit=args.limit, seed=args.seed), ensure_ascii=False, indent=2))
