...
function formatOfficialDetail(rawText: string): string {
  try {
    ...
    add("职位", payload.title);
    add("公司", payload.company);
    add("地点", payload.location);
    add("部门", payload.department);
    add("招聘类型", payload.employment_type);
    add("发布时间", payload.published_at);
    if (typeof payload.description === "string" && payload.description.trim()) lines.push("\n职位描述\n" + payload.description.trim());
    const requirements = Array.isArray(payload.requirements) ? payload.requirements.filter((item): item is string => typeof item === "string" && Boolean(item.trim())) : [];
    if (requirements.length > 0) lines.push("\n任职要求\n" + requirements.map((item) => `- ${item.trim()}`).join("\n"));
...
export function OfficialSourcesPanel() {
  ...
  async function admitAndPrepare(item: JobStagingRecord) {
    ...
    try {
      ...
      setDetails((current) => ({ ...current, [item.staging_id]: formatOfficialDetail(detail.raw_text) }));
      ...
      const detailRequirements = detail.normalized.requirements.filter((text) => text.trim());
      const candidateRequirements = detailRequirements.length > 0 ? detailRequirements : item.normalized.requirements;
      for (const [index, text] of candidateRequirements.entries()) {
      ...
      setMessage(candidateRequirements.length > 0 ? "岗位已加入求职流程；完整官方 JD 已留存，候选要求已生成，请逐条审核。接受要求前需绑定正式能力。" : "岗位已加入求职流程，但官方详情未解析出任职要求；请查看完整 JD 后手动补充候选要求。");
    } catch (error) {
      setMessage(localizedApiError(error));
    } finally {
      setBusy(null);
